...
static inline rcc_peripheral_clock_en_t __get_timer_clock(TIM_TypeDef *timer) {
...
static inline gpio_alternate_function_t __get_timer_alternate_function(TIM_TypeDef *timer) {
...
static inline uint32_t __get_timer_clock_source_frequency(TIM_TypeDef *timer) {
...
void timer_init_counter(TIM_TypeDef *timer, uint32_t frequency_hz, timer_dir_t direction, uint32_t reload_value) {
    ...
    timer->PSC = __get_timer_clock_source_frequency(timer) / frequency_hz - 1;
...
void timer_init_pwm(TIM_TypeDef *timer, uint8_t channel, GPIO_TypeDef *port, uint8_t gpio, uint32_t frequency_hz, uint32_t reload_value) {
    ...
    timer->PSC = __get_timer_clock_source_frequency(timer) / ((reload_value + 1) * frequency_hz) - 1;
    timer->ARR = reload_value;      // auto-reload value
